...
class LabelGenerator:
    ...
    def generate_pdf(self):
        ...
        pair_height = 2.5 * inch # Full pair height
        ...
        for i, item in enumerate(self.queue):
            ...
            # Nametag overlays (fixed THC, no shift)
            x_n = x_left + 0.1 * inch
            y_n = y - 0.2 * inch
            ...
            pdf.drawCentredString(x_n + label_width / 2, y_n, strain.name.upper()) # Centered
            y_n -= 0.2 * inch
            if brand['category'] == 'REC':
                pdf.line(x_n, y_n, x_n + label_width - 0.5 * inch, y_n)
            y_n -= 0.2 * inch
            ...
            pdf.drawCentredString(x_n + label_width / 2, y_n - 0.3 * inch, f"THC: {strain.thc_percent}%") # Fixed bottom-center
            # Pricetag overlays (none, just background)
            pdf.setFillColor(black)
        pdf.save()
